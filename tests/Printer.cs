...
namespace Hospital_information_sytem
{
    ...
    internal static class BTreePrinter
    {
        ...
        public static void Print(this Node<int, String> root, int topMargin = 2, int leftMargin = 2)
        {
            ...
            {
                var item = new NodeInfo { Node = next, Text = next.Key.ToString() }; // v zatvorke bolo " 0 "
                if (level < last.Count)
                {
        ...
        /*public static void Print(this Node<int, String> root, string textFormat = "0", int spacing = 1, int topMargin = 2, int leftMargin = 2)
        {
            if (root == null) return;
            int rootTop = Console.CursorTop + topMargin;
            var last = new List<NodeInfo>();
